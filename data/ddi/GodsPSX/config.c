...
void Config_BreakSpace(LPUCHAR s)
{
	int i, len;
	len = strlen(s);
	for(i=0;i<len;i++)
...
lpConfig	Config_Load(LPUCHAR filename)
{
	...
	LPUCHAR	s, fdata;
	UCHAR		c;
	BOOL		commentMode = FALSE, stringMode=FALSE;
	...
	if (fdata = File_LoadBinary(filename, &fileSize))
	{
		...
		// Added support for strings with spaces (enclosed in quotes) 3/6/99 -ADRIAN-
		for (s=rootConf->fileData,loop=0; loop<fileSize; loop++)
		{
			...
			if (*s == '"' && !commentMode) stringMode ^= 1, *s = '\0';
		...
		s=fdata;
		while(fileSize>((int)s-(int)fdata))
		{
			if (('\0' != *s) && (' ' != *s))
			{
				if ('}' == *s)
				...
				{
					conf->dataString = s;
//					if(conf->itemName != NULL)
//						printf("Config create item name %s, data %s\n",conf->itemName,conf->dataString);
...
{
	int i;
	for(i=0;i<configGlobs.listCount;i++)
	{
		if(configGlobs.listSet[i] != NULL)
		{
			Mem_Free(configGlobs.listSet[i]);
			configGlobs.listSet[i] = NULL;
		}
	}
}
...
REAL	Config_GetRealValue(lpConfig root, LPUCHAR stringID)
{
	...
	if (negative == TRUE)	return -((retval*ONE)/loop);
