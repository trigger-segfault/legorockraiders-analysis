...
void	Image_WADDownloadTIM(wad_handler *handle,LPUCHAR name)
{
	...
	if(buffer = WAD_LoadFile(handle,name,FALSE))
	{
		OpenTIM((Uint32*)buffer);
		ReadTIM(&tim);
		LoadImage(tim.crect, tim.caddr);
...
/*
	download CLUT from TIM to VRAM only
*/
...
lpImage	Image_Clone(lpImage img)
{
	...
	newImage = Mem_Malloc(sizeof(Image));
...
BOOL	Image_DisplaySpriteIndex(SPRITE_SET *set, int frame_offset, int x, int y, int scale, int order, Uint8 clutIndex, Uint8 brightness, int clipBottom)
{
	...
	if (set)
	{
		...
		scale = Asm_MulFIX((scale<<12), (IMAGE_GetScale())) >> 12;
...
//	display_quad->b0=cont->blue;
	...
	if (!(cont->renderFlags & RENDER_FLAGS_INVISIBLE))
	{
...
{
// I know, I know but I was getting hacked off with recompiling and waiting for the game to load
// so I did this.  It isn't even that accurate but the amount we scale is so little as long
// as it looks smaller it'll probably do.  If I get chance to use a debugger I'll do it properly
// but if no-one complains......hee hee
	...
	if ((image_zoom >  4000) && (image_zoom <= 4200))	return ToFIXED(0.98);
	if ((image_zoom >  4200) && (image_zoom <= 4400))	return ToFIXED(0.96);
	if ((image_zoom >  4400) && (image_zoom <= 4600))	return ToFIXED(0.94);
	if ((image_zoom >  4600) && (image_zoom <= 4800))	return ToFIXED(0.92);
	if ((image_zoom >  4800) && (image_zoom <= 5000))	return ToFIXED(0.90);
	if ((image_zoom >  5000) && (image_zoom <= 5200))	return ToFIXED(0.88);
	if ((image_zoom >  5200) && (image_zoom <= 5400))	return ToFIXED(0.86);
	if ((image_zoom >  5400) && (image_zoom <= 5600))	return ToFIXED(0.84);
	if ((image_zoom >  5600) && (image_zoom <= 5800))	return ToFIXED(0.82);
	if ((image_zoom >  5800) && (image_zoom <= 6000))	return ToFIXED(0.80);
	if  (image_zoom >  6000)							return ToFIXED(0.75);
