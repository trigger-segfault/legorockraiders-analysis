...
lpContainer Container_getContainer(VOID)
{
	...
	for (n=0; n<MAX_CONTAINERS; n++)
	{
		if (containerGlobs.listSet[n]->in_use==FALSE)
		{
			containerGlobs.listSet[n]->in_use=TRUE;
//			containerGlobs.list_index=offset+1;
			return	containerGlobs.listSet[n];
		}
//		offset++;
	}
...
*/
	/* Cannot find a way to find out if this is a CD version	*/
	/* So I'm just printing it									*/
	printf("Container failure - limit of %d exceeded\n", MAX_CONTAINERS); 
...
//{
	LPDIRECT3DRMMESH mosh = (LPDIRECT3DRMMESH) dead->modelData;
	HMD_Model *model = mosh->model;
...
{
/*
	lpContainer	childOfParent=parent->next;

	if (child)
	{
		child->parent=parent;
		parent->next=child;
		child->next=childOfParent;
	...
	else	// remove container from chain
	{
		lpContainer		parentOfParent=parent->parent;

		parentOfParent->next=childOfParent;

		if (childOfParent)		// could be NULL if end of list
			childOfParent->parent=parentOfParent;
	}
*/
}
...
VOID Container_SetRotation(lpContainer cont, REAL x, REAL y, REAL z)
{
	...
	if ((cont->type == TYPE_HMD) || (cont->type == TYPE_STATIC_HMD))
	{
...
/*
	SteveD: new routine added here, 'cos the other stuff makes no sense at all 
*/
...
//	printf ("loaded binary\n");
	if (load_file)
	{
		hmd_model=HMD_InitModel(load_file,NULL,modeflags);
...
// ***********************************************************************************************
// Added LoadStaticHMDModel - ScottN
...
lpContainer Container_Clone(lpContainer orig)
{
	...
	if ((orig->type == TYPE_HMD) || (orig->type == TYPE_STATIC_HMD))
	{
...
BOOL Container_SetActivityByType(lpContainer cont, int activityType)
{
	...
	if ((cont->type & TYPE_DECAL)==TYPE_DECAL)		/* sprites, or floor sprites */
	{
		...
		if (sad->activitySet->seq_list[activityType])
...
void Container_UpdateAnimation(lpContainer cont, int update)
{
	...
	{
		if (index<0)		/* animation can go backwards */
		{
			sad->animTotal+=(set->loop_seq_len<<12);
			index+=set->loop_seq_len;
		}	
	...
	// BEGIN ScottN
	// If the animation sequence is to return to the last one
	// probably the biggest botch job in the world.....
	frame = set->frame_sequence[sad->animIndex];
    if ((frame >= 200) && (frame <= 208))
	{
		Container_SetActivityByType(cont, frame-200);
	}
	else
	{
		if ( frame == 254)
		{
			Container_SetActivityByType(cont, cont->old_activity_seq);
			cont->old_activity_seq = cont->activitySeq;
		}
	}
	// END ScottN
...
void	Container_SetScaleBlockSeparate(lpContainer cont, int index, int scaleX, int scaleY, int scaleZ)
{
	if ((cont->type==TYPE_HMD) || (cont->type == TYPE_STATIC_HMD))
	{
		VECTOR		mesh_scale={scaleX,scaleY,scaleZ};
...
}/* Container_GetScreenY */


// *****************************************************************************************
// BEGIN ScottN
// Need to know when we've passed the end of an animation
...
lpSPRITE_ANIM_DATA		sad=(SPRITE_ANIM_DATA*) cont->userData;
	...
	*anim_frame = set->frame_sequence[sad->animTotal>>12];
	
	*max_frame = set->loop_seq_len;
}

// *****************************************************************************************
// return which activity we are using.  
short Container_GetActivity( lpContainer cont )
{
SPRITE_SET				*set=(SPRITE_SET*) cont->modelData;
lpSPRITE_ANIM_DATA		sad=(SPRITE_ANIM_DATA*) cont->userData;

	return cont->activitySeq;
}

// *****************************************************************************************
// set our sprite callback function
void Container_SetCallback(lpContainer cont, short frame, void (*Callback)(void))
{
	cont->callback			=	Callback;
	cont->callback_frame	=	frame;
}
