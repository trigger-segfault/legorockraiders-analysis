// CFG.cpp : Defines LegoRR CFG configuration file functions.
//
...
namespace CFG {
	...
	lego::pool::ReservedPool<CFGProperty, 32> Pool;
...
BOOL __cdecl lego::CFG::IsKeyMatch(const CFGProperty* prop, const char* keyName, BOOL* out_isWildcard)
{
...
// 
// <LegoRR.exe @004795a0>
CFGProperty* __cdecl lego::CFG::GetProperty(CFGProperty* prop, const char* keyPath)
{
	...
	while (prop != nullptr) {
		...
		if (prop->depth != depth || !IsKeyMatch(prop, parts[depth], &isWildcard)) {
			prop = prop->next;
			continue;
		}
		...
		while (previous != nullptr) {
			if (previous->depth == depth - 1) { // is this property the starting block of previous?
				if (!IsKeyMatch(previous, parts[depth - 1], &isWildcard))
...
// <LegoRR.exe @004792b0>
CFGProperty* __cdecl lego::CFG::GetBlockFirstChild(CFGProperty* prop, const char* keyPath)
{
	prop = GetProperty(prop, keyPath);
...
// <LegoRR.exe @00479370>
const char* __cdecl lego::CFG::GetPropertyValue(CFGProperty* prop, const char* keyPath)
{
	prop = GetProperty(prop, keyPath);
	if (prop != nullptr)
		return prop->value;
...
// <LegoRR.exe @00479310>
char* __cdecl lego::CFG::CopyPropertyValue(CFGProperty* prop, const char* keyPath)
{
	prop = GetProperty(prop, keyPath);
...
// <LegoRR.exe @004792e0>
CFGProperty* __cdecl lego::CFG::NextFlatProperty(CFGProperty* prop)
{
...
// 
// <LegoRR.exe @00479430>
BOOL __cdecl lego::CFG::GetPropertyRGBF(CFGProperty* prop, const char* keyPath, float* out_r, float* out_g, float* out_b)
{
	char* value = CopyPropertyValue(prop, keyPath);
	if (value != nullptr) {
...
// 
// <LegoRR.exe @004793d0>
float __cdecl lego::CFG::GetPropertyRadians(CFGProperty* prop, const char* keyPath)
{
	const char* propValue = GetPropertyValue(prop, keyPath);
	if (propValue == NULL) {
		propValue = ""/*EMPTYSTR*/;
	}
	else {
		propValue = GetPropertyValue(prop, keyPath);
	}
...
// 
// <LegoRR.exe @00479390>
lego::BOOL3 __cdecl lego::CFG::GetPropertyBool(CFGProperty* prop, const char* keyPath)
{
...
// <LegoRR.exe @00479530>
CFGProperty* __cdecl lego::CFG::CFG__Next(CFGProperty* previous)
{
	...
	/*pool::PoolItem<CFGProperty>* poolProp = Pool.g_NEXT;
	pool::PoolItem<CFGProperty>* poolNext = Pool.g_NEXT->m_next;
	...
	CFGProperty* prop = &poolProp.m_item;*/

	CFGProperty* prop = Pool.Next();
...
void __cdecl lego::CFG::ZeroWhiteSpace(char* text, unsigned int length)
{
...
// 
// <LegoRR.exe @00479120>
CFGProperty* __cdecl lego::CFG::Open(const char* filename)
{
	...
	CFGProperty* root = CFG__Next(nullptr); // NULL, root property
	root->fileText = text;
	if (length == 0)
...
// 
// <LegoRR.exe @00479500>
void __cdecl lego::CFG::Close(CFGProperty* root)
{
