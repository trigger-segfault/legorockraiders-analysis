...
namespace globals {
	...
	// <LegoRR.exe @00576500>
	static WADFileClass g_WADFILES_TABLE[WADFILES_COUNT /*10*/];
	...
	// <LegoRR.exe @005766a0>
	static SharedWADEntryClass g_SHARED_WADENTRIES_TABLE[SHARED_WADENTRIES_COUNT /*100*/]; // isn't this too small???
...
#define WAD_Get lego::file::WADFileClass::Get
...
void __cdecl lego::file::InitFileSystem(const char* gamename, BOOL insistOnCD, const char* regKey)
{
	...
	for (int i = 0; i < WADFILES_COUNT /*10*/; i++) {
		...
		if (WADFileClass::Open(wadFile) != -1) { // -1 returned for no WAD index (index does not correspond to file number)
			hasWADs = TRUE;
		}
	}
...
//
// <LegoRR.exe @0047f900>
int __cdecl lego::file::WADFileClass::Open(const char* filename)
{
	//lego::logf_removed(nullptr);
	//return Open__internal(filename);
	return Open__internal_wrapper(filename);
}
...
// <LegoRR.exe @0047f9a0>
FileStreamClass* __cdecl lego::file::FileStreamClass::Open(const char* in_filename, const char* mode)
{
	char buffer[MAX_PATH /*260*/];

	const char* message;
	const char* filename = Path_JoinDataDir(in_filename);
	FileLocation location = FindLocation(filename, mode);
	...
	fileStream->File__Dispose();
	return nullptr;
}

// <LegoRR.exe @0047fb10>
int lego::file::FileStreamClass::Seek(long int offset, int origin)
{
	FileLocation location = this->GetLocation();
	if (location == FILELOC_WAD /*0*/) {
		switch (origin) {
		case SEEK_SET /*0*/:
			this->wad->position = offset;
		...
		case SEEK_END /*2*/:
			this->wad->position = (int)WAD_SharedGetSize(this->wad->sharedIndex) + offset;
		...
		// bounds checks
		if (this->wad->position > (int)WAD_SharedGetSize(this->wad->sharedIndex))
			this->wad->position = WAD_SharedGetSize(this->wad->sharedIndex);
		else if (this->wad->position < 0)
			this->wad->position = 0;
		// despite what you'd think, this always returns 0 for WADs, but the EAX result register is clearly set...
		return 0;
	}
	else if (location == FILELOC_STD /*1*/) {
		return std::fseek(this->std, offset, origin);
	}
	else {
...
// <LegoRR.exe @0047fc40>
size_t lego::file::FileStreamClass::Read(void* out_ptr, size_t size, size_t count)
{
	FileLocation location = this->GetLocation();
	if (location == FILELOC_WAD /*0*/) {
		size_t fileSize = (int)WAD_SharedGetSize(this->wad->sharedIndex);
		size_t readSize = count * size;
		int position = this->wad->position;
		if ((int)fileSize < (position + readSize))
			readSize = fileSize - position;
		unsigned char* fileData = WAD_SharedGetData(this->wad->sharedIndex);
		std::memcpy(out_ptr, (fileData + this->wad->position), readSize);
		this->wad->position += readSize;
		return (int)readSize / (int)size; // (partial element size reads may occur)
	}
	else if (location == FILELOC_STD /*1*/) {
		return std::fread(out_ptr, size, count, this->std);
	}
	else {
...
// <LegoRR.exe @0047fd10>
size_t lego::file::FileStreamClass::Write(const void* ptr, size_t size, size_t count)
{
	FileLocation location = this->GetLocation();
	if (location == FILELOC_WAD /*0*/) {
	...
	if (location == FILELOC_STD /*1*/) {
		return std::fwrite(ptr, size, count, this->std);
	}
	else {
...
// <LegoRR.exe @0047fd80>
int lego::file::FileStreamClass::Close()
{
	FileLocation location = this->GetLocation();
...
// <LegoRR.exe @0047fdd0>
long int lego::file::FileStreamClass::Tell() const
{
	FileLocation location = this->GetLocation();
	if (location == FILELOC_WAD /*0*/) {
		return (long int)this->wad->position;
		// return *(int *)(file->stream + 4);
	}
	if (location == FILELOC_STD /*1*/) {
		return std::ftell(const_cast<FILE*>(this->std));
	}
...
// <LegoRR.exe @0047fe20>
BOOL __cdecl lego::file::FileStreamClass::Exist(const char* filename)
{
	char buffer[MAX_PATH /*260*/];

	const char* dataFilename = Path_JoinDataDir(filename);
	FileLocation location = FindLocation(dataFilename, "r");
	if (location == FILELOC_WAD /*0*/) {
...
// <LegoRR.exe @0047fee0>
int lego::file::FileStreamClass::GetC()
{
	FileLocation location = this->GetLocation();
	if (location == FILELOC_WAD /*0*/) {
		if ((int)(WAD_SharedGetSize(this->wad->sharedIndex) - 1) <= this->wad->position)
			return -1; // end of stream

		unsigned char* fileData = WAD_SharedGetData(this->wad->sharedIndex);
		return fileData[this->wad->position++];
	}
	else if (location == FILELOC_STD /*1*/) {
		return std::fgetc(this->std);
	}
	else {
...
// <LegoRR.exe @0047ff60>
long int lego::file::FileStreamClass::GetLength()
{
	int origOffset = this->Tell();
	this->Seek(0, SEEK_END /*2*/);
	long int length = this->Tell();
	this->Seek(origOffset, SEEK_SET /*0*/);
	return length;
}

// <LegoRR.exe @0047ffa0>
char* lego::file::FileStreamClass::WAD_GetS__internal(char* out_str, int num)
{
	int length = 0;
	char c = (char)this->GetC();
	out_str[0] = c;
...
// <LegoRR.exe @00480000>
char* lego::file::FileStreamClass::GetS(char* out_str, int num)
{
	FileLocation location = this->GetLocation();
	if (location == FILELOC_WAD /*0*/) {
		return this->WAD_GetS__internal(out_str, num);
	}
	else if (location != FILELOC_STD /*1*/) {
		return std::fgets(out_str, num, this->std);
	}
	else {
...
// <LegoRR.exe @00480070>
int lego::file::FileStreamClass::PrintF(const char* format, ...)
{
	FileLocation location = this->GetLocation();
	if (location == FILELOC_WAD /*0*/) {
		...
		va_list vl;
		va_start(vl, format);
		std::vfprintf(this->std, format, vl);
...
// <LegoRR.exe @004800e0>
FileLocation lego::file::FileStreamClass::GetLocation() const
{
...
// 
// <LegoRR.exe @004800f0>
FileLocation __cdecl lego::file::FileStreamClass::FindLocation(const char* filename, const char* mode)
{
...
#define File__New__alloc std::malloc
#define File__New__free std::free

// <LegoRR.exe @00480190>
FileStreamClass* __cdecl lego::file::FileStreamClass::File__New(FileLocation location)
{
	if (location == FILELOC_WAD /*0*/) {
		FileStreamClass* file = (FileStreamClass*)File__New__alloc(sizeof(FileStreamClass) /*8*/);
		if (file != nullptr) {
...
// <LegoRR.exe @00480210>
void lego::file::FileStreamClass::File__Dispose()
{
	if (this != nullptr) {
		FileLocation location = this->GetLocation();
		if (location == FILELOC_WAD /*0*/) {
			if (this->wad != nullptr) {
				WAD_Shared__Dispose(this->wad->sharedIndex);
				File__New__free(this->wad);
			}
			File__New__free(this);
		}
		else if (location == FILELOC_STD /*1*/) {
			if (this->std != nullptr) {
				std::fclose(this->std);
...
// 
// <LegoRR.exe @00480310>
char* __cdecl lego::file::FileStreamClass::GetS_StripLineEnd(char* out_str, int num)
{
	char* result = this->GetS(out_str, num);
...
// <LegoRR.exe @00480360>
unsigned char* __cdecl lego::file::FileStreamClass::ReadAllBytes(const char* filename, unsigned int* out_length)
{
	return ReadAll(filename, out_length, TRUE /*isBinary*/);
}

// <LegoRR.exe @00480380>
unsigned char* __cdecl lego::file::FileStreamClass::ReadAll(const char* filename, unsigned int* out_length, BOOL isBinary)
{
	FileStreamClass* file = FileStreamClass::Open(filename, isBinary ? "rb" : "r");
	if (file != nullptr) {
		file->Seek(0, SEEK_END /*2*/);
...
{
	FileStreamClass* file = File_Open(filename, "rb");
	if (file != nullptr) {
...
// <LegoRR.exe @00480570>
void __cdecl lego::file::FileStreamClass::SetOpenCallback(FileOpenCallback callback, void* lpContext)
{
...
// <LegoRR.exe @00480590>
void __cdecl lego::file::Path_ScanRealFiles__UNK__00480590(const char* filename)
{
	///TODO: decompile me
}

// <LegoRR.exe @00480650>
void __cdecl lego::file::File_Find(const char* dirname)
{
	char buffer[1024];
	...
	if (handle != -1) {
		do {
			...
			if ((finddata.attrib & FILE_ATTRIBUTE_DIRECTORY /*0x10*/) == 0) {
...
// <LegoRR.exe @00480830>
void __cdecl lego::file::FUN_00480830(const char* filename)
{
	///TODO: decompile me
}
...
// 
// <LegoRR.exe @0048b7a0>
int __cdecl lego::file::WADFileClass::Open__internal_wrapper(const char* filename)
{
	...
	WADFileClass* wad = &globals::g_WADFILES_TABLE[idx];
	wad->Open__internal(filename);
}
...
// 
// <LegoRR.exe @0048b7a0>
int __cdecl lego::file::WADFileClass::Open__internal(const char* filename)
{
	char signature[4];
	FILE* file;
	int idx = WAD__Next(); // prepare and get next available WAD index
...
// 
// <LegoRR.exe @0048bfa0>
WADFileClass* __cdecl lego::file::WADFileClass::Get(int wadIndex)
{
...
int __cdecl lego::file::WAD__Next(void)
{
	for (int i = 0; i < WADFILES_COUNT /*10*/; i++) {
		if (!globals::g_WADFILES_TABLE[i].isUsed) { // NOTE: field_4 is checked, not field_0
			...
			std::memset(&globals::g_WADFILES_TABLE[i], 0, sizeof(WADFileClass));
			return i;
		}
	}
	return -1;
...
// 
// <LegoRR.exe @0048c100>
int __cdecl lego::file::SharedWADEntryClass::SharedOpen__InWAD(const char* filename, int wadIndex)
{
	int idx = Shared__Next();
	if (idx == -1)
		return -1;

	int entryIndex = WADFileClass::FindEntry(filename, wadIndex);
	if (entryIndex == -1)
		return -1;

	size_t entrySize = WADFileClass::GetEntryPackedSize(wadIndex, entryIndex);
	...
	entrySize = WADFileClass::GetEntryPackedSize(wadIndex, entryIndex);
...
// 
// <LegoRR.exe @0048c230>
int lego::file::SharedWADEntryClass::SharedOpen(const char* filename, int opt_wadIndex)
{
	if (opt_wadIndex != -1)
		return SharedOpen__InWAD(filename, opt_wadIndex);

	for (int wadIndex = WADFILES_COUNT - 1 /*10-1=9*/; wadIndex >= 0; wadIndex--) {
		if (!WAD_Get(wadIndex)->isUsed)
			continue;

		int sharedIndex = SharedOpen__InWAD(filename, wadIndex);
		if (sharedIndex != -1)
			return sharedIndex;
	}
	return -1;
...
// <LegoRR.exe @0048c280>
void __cdecl lego::file::WADFileClass::Shared__Dispose(int sharedIndex)
{
...
// <LegoRR.exe @0048c2b0>
unsigned char* __cdecl lego::file::WADFileClass::SharedGetData__internal(int sharedIndex)
{
	if (globals::g_SHARED_WADENTRIES_TABLE[sharedIndex].isUsed) {
...
// <LegoRR.exe @0048c2d0>
unsigned int __cdecl lego::file::WADFileClass::SharedGetSize(int sharedIndex)
{
	return GetEntrySize(globals::g_SHARED_WADENTRIES_TABLE[sharedIndex].wadIndex,
		globals::g_SHARED_WADENTRIES_TABLE[sharedIndex].entryIndex);
}

// <LegoRR.exe @0048c2f0>
unsigned char* __cdecl lego::file::WADFileClass::SharedGetData(int sharedIndex)
{
	return SharedGetData__internal(sharedIndex);
}
